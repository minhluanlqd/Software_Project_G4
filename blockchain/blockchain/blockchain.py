...
def buildBlocks():
	...
	for i in range(len(ledgerdata)):
		...
		if(i == 0):
			continue
		blockChain.append(Block(blockChain[-1].hash, int(transaction[0]), int(transaction[1]), i))	
		if(blockChain[i].hash != hashdata[i].strip('\n')): 
			print("The ledger was tampered with")
			break
...
def addBlock(transNum, userID, cost): 
	...
	with open('hashtable.txt', 'r') as hashFile: 
		...
		if(len(ledgerdata) < (transNum+1)):
			blockChain.append(Block(blockChain[-1].hash, int(userID), int(cost), int(transNum)))
...
def getBalance(user):
	...
	for i in range(len(data)): 
		if(int((decrypt(data[i].split()).split())[0]) == user):
			amount += int((decrypt(data[i].split()).split())[1])
	return amount
